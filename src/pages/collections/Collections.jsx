...
export const Collections = ({ currentUser, contract }) => {
  ...
  useEffect(() => {
    const allCollectionsPromise = new Promise(async (resolve, reject) => {
      const collections = await contract
        .get_collections()
    ...
    const userCollectionsPromise = new Promise(async (resolve, reject) => {
      const userCollectionCounts = await contract
    ...
    Promise.all([allCollectionsPromise, userCollectionsPromise]).then(
      (result) => {
            ...
            {isReady ? (
              <div className="flex flex-row flex-wrap text-left">
                {allCollections.map((collection) => (
                  <div
                    className="basis-1/2 my-10 flex gap-7"
                    key={collection.id}
                  >
                    ...
                    <div className="w-2/3">
                      ...
                      <div className="pr-16 mb-5 mt-1 text-sm font-normal">
                        Your collection of stones. Later you will be able to
                        open and see all collection items and your own NFTs.
                      </div>
                    </div>
                  </div>
                ))}
